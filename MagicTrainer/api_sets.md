...
Due to Magic’s long and complicated history, Scryfall includes many un-official
sets as a way to group promotional or outlier cards together.
...
| `code` | String |  | The unique three to six-letter code for this set. |
...
| `tcgplayer_id` | Integer | Nullable | This set’s ID on [TCGplayer’s API](https://docs.tcgplayer.com/docs), also known as the `groupId`. |
...
Scryfall provides an overall categorization for each Set in the
`set_type` property. An exhaustive list of `set_types` is listed below:

Type | Description || `core` | A yearly Magic core set (Tenth Edition, etc) |
...
https://api.scryfall.com/sets/aer