...
Card objects represent individual *Magic: The Gathering* cards that
players could obtain and add to their collection (with a few minor exceptions).
...
Each unique Oracle name is separately available in the
[card names catalog](https://scryfall.com/docs/api/catalogs/card-names).

Note that while most Oracle card names are unique,
Scryfall also indexes other objects such as tokens and Unstable set variants
which do not always have a unique name.

## Multiface Cards

Magic cards can have multiple faces or multiple cards printed on one card stock.
The faces could be shown divided
on the front of the card as in
[split cards](/search?q=is%3Asplit)
and [flip cards](/search?q=is%3Aflip),
or the card can be double-faced as in
[transform cards](/search?q=is%3Atransform)
and [modal DFCs](/search?q=is%3Amdfc).
...
| `lang` | String |  | A [language](/docs/api/languages) code for this printing. |
...
| `resource_id` | String | Nullable | This card’s Resource ID on Gatherer, if any. |
| `tcgplayer_id` | Integer | Nullable | This card’s ID on [TCGplayer’s API](https://docs.tcgplayer.com/docs), also known as the `productId`. |
| `tcgplayer_etched_id` | Integer | Nullable | This card’s ID on [TCGplayer’s API](https://docs.tcgplayer.com/docs), for its etched version if that version is a separate product. |
...
| `layout` | String |  | A code for this card’s [layout](/docs/api/layouts). |
...
| `rulings_uri` | URI |  | A link to this card’s [rulings list](/docs/api/rulings) on Scryfall’s API. |
...
| Property | Type | Atn | Details |
| --- | --- | --- | --- |
| `all_parts` | Array | Nullable | If this card is closely related to other cards, this property will be an array with [Related Card Objects](#related-card-objects). |
| `card_faces` | Array | Nullable | An array of [Card Face](#card-face-objects) objects, if this card is multifaced. |
| `cmc` | Decimal |  | The card’s mana value. Note that some funny cards have fractional mana costs. |
| `color_identity` | [Colors](/docs/api/colors) |  | This card’s color identity. |
| `color_indicator` | [Colors](/docs/api/colors) | Nullable | The colors in this card’s color indicator, if any. A null value for this field indicates the card does not have one. |
| `colors` | [Colors](/docs/api/colors) | Nullable | This card’s colors, if the overall card has colors defined by the rules. Otherwise the colors will be on the `card_faces` objects, see below. |
...
| `game_changer` | Boolean | Nullable | True if this card is on the [Commander Game Changer list](https://mtg.wiki/page/Commander_(format)/Game_Changers). |
...
| `mana_cost` | String | Nullable | The mana cost for this card. This value will be any empty string `""` if the cost is absent. Remember that per the game rules, a missing mana cost and a mana cost of `{0}` are different values. Multi-faced cards will report this value in [card faces](#card-face-objects). |
...
| `produced_mana` | [Colors](/docs/api/colors) | Nullable | Colors of mana that this card could produce. |
...
| `attraction_lights` | Array | Nullable | The lit [Unfinity attractions](/search?q=t%3Aattraction+unique%3Aprints) lights on this card, if any. |
| `booster` | Boolean |  | Whether this card is found in boosters. |
| `border_color` | String |  | This card’s border color: `black`, `white`, `borderless`, `yellow`, `silver`, or `gold`. |
...
| `content_warning` | Boolean | Nullable | True if you should consider [avoiding use of this print](https://scryfall.com/blog/220) downstream. |
...
| `frame_effects` | Array | Nullable | This card’s [frame effects](/docs/api/frames), if any. |
| `frame` | String |  | This card’s [frame layout](/docs/api/frames). |
| `full_art` | Boolean |  | True if this card’s artwork is larger than normal. |
| `games` | Array |  | A list of games that this card print is available in, `paper`, `arena`, `mtgo`, `astral`, and/or `sega`. |
...
| `image_uris` | Object | Nullable | An object listing available imagery for this card. See the [Card Imagery](/docs/api/images) article for more information. |
...
| `set_uri` | URI |  | A link to this card’s [set object](/docs/api/sets) on Scryfall’s API. |
...
Card Face objects. Those objects have the following properties:
...
| `color_indicator` | [Colors](/docs/api/colors) | Nullable | The colors in this face’s color indicator, if any. |
| `colors` | [Colors](/docs/api/colors) | Nullable | This face’s colors, if the game defines colors for the individual face of this card. |
| `defense` | String | Nullable | This face’s defense, if any. |
...
or generate a token, or meld, etc) have a `all_parts` property
that contains Related Card objects. Those objects have the following properties:
