...
With the server running (see [Getting Started](getting-started.html)):
...
Responses use **camelCase** keys (a middleware converts Django's snake\_case), which is what the generated clients and the frontend expect.
...
| --- | --- |
| `GET /variants/` | The generated [variants](domain-model.html#variant) — the main read endpoint. Supports the [search query language](#the-search-query-language). |
| `GET /cards/` | Cards. |
| `GET /features/` | Features. |
| `GET /templates/` | Templates. |
| `GET`/`POST /find-my-combos` | Given a decklist, returns the combos it can assemble (the engine's [up phase](variant-generation.html#up-phase--find-combos-from-a-hand-bfs-from-cards)). |
...
* **Python** — package `spellbook_client` (async, `asyncio` library). Used by the [bots](architecture.html#repository-layout) and the Python integration tests.
...
Authors: [ldeluigi](mailto:44567586+ldeluigi@users.noreply.github.com)